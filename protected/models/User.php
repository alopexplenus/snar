...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('gender, username, password, email', 'required'),
			array('gender', 'numerical', 'integerOnly'=>true),
			array('username, password, salt, email', 'length', 'max'=>128),
			array('profile', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, gender, username, email, profile', 'safe', 'on'=>'search'),
		);
	}
	...
	protected function beforeSave()
	{
		...
		{
			if($this->isNewRecord) // я не дебил, оставил ветвление чтобы не забыть как называется свойство
			{
				$n = rand(10e16, 10e20);
				$this->salt = base_convert($n, 10, 36);
